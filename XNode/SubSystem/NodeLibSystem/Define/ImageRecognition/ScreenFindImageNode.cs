...
namespace XNode.SubSystem.NodeLibSystem.Define.ImageRecognition
{
    ...
    /// 在整个屏幕范围内查找单个匹配结果
    /// </summary>
    public class ScreenFindImageNode : NodeBase
    {
        #region Win32 API
        ...
        private const int PIN_GROUP_FOUND_X = 3;
        private const int PIN_GROUP_FOUND_Y = 4;
        private const int PIN_GROUP_FOUND_CONFIDENCE = 5;
        private const int PIN_GROUP_ACTION_FOUND = 6;
        private const int PIN_GROUP_ACTION_NOT_FOUND = 7;
        ...
        protected override void ExecuteNode()
        {
            try
            {
                ...
                {
                    Console.WriteLine("[ScreenFindImageNode] 模板图像路径为空");
                    ExecuteNotFound();
                    return;
                }
                ...
                {
                    Console.WriteLine($"[ScreenFindImageNode] 模板图像文件不存在: {templatePath}");
                    ExecuteNotFound();
                    return;
                }

                Console.WriteLine($"[ScreenFindImageNode] 屏幕查找图像: {Path.GetFileName(templatePath)}, 阈值: {threshold:F2}");
                ...
                // 执行模板匹配
                var result = TemplateMatch(screenBitmap, templateBitmap, threshold);
                ...
                if (result.HasValue)
                {
                    ...
                    Console.WriteLine($"[ScreenFindImageNode] 匹配成功, 位置: ({result.Value.X}, {result.Value.Y}), 置信度: {result.Value.Confidence:F3}");
                    ExecuteFound();
                }
                else
                {
                    ResetOutputs();
                    Console.WriteLine("[ScreenFindImageNode] 未找到匹配图像");
                    ExecuteNotFound();
                }
            }
            catch (Exception ex)
            {
                Console.WriteLine($"[ScreenFindImageNode] 执行屏幕找图时发生错误: {ex.Message}");
                ResetOutputs();
                ExecuteNotFound();
                throw;
            }
        ...
        public override string GetTypeString() => nameof(ScreenFindImageNode);
        ...
        protected override NodeBase CloneNode() => new ScreenFindImageNode();
